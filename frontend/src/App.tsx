...
import type { AIPlan, Bootstrap, HistoryItem, Stats, Tags, Track } from "./types";

type View = "library" | "add" | "assistant" | "history" | "quarantine";
...
function LibraryView(props: LibraryProps) {
  return <div className="library-layout">
    ...
    <section className={props.selected ? "editor-pane open" : "editor-pane"}>
      {props.selected ? <TrackEditor track={props.selected} onClose={() => props.onSelect(null)} onUpdated={props.onUpdated} onError={props.onError} /> : <div className="bench-empty"><div className="record-grooves"><Disc3 /></div><span>Track bench</span><p>Select a cut to inspect its label, hear it, and prepare corrections.</p></div>}
    </section>
  </div>;
}
...
function TrackRow({ track, active, checked, onCheck, onClick }: { track: Track; active: boolean; checked: boolean; onCheck: () => void; onClick: () => void }) {
  return <button className={active ? "cue-row active" : "cue-row"} onClick={onClick}>
    ...
    <span className={track.issues.length ? "condition needs" : "condition clean"}>{track.issues.length ? `${track.issues.length} ${track.issues.length === 1 ? "issue" : "issues"}` : "Clean"}</span>
    <span className="duration">{formatDuration(track.duration)}<ChevronRight /></span>
  </button>;
}

function TrackEditor({ track, onClose, onUpdated, onError }: { track: Track; onClose: () => void; onUpdated: (track: Track, message: string) => Promise<void>; onError: (error: unknown) => void }) {
...
function AddView({ capabilities, onAdded, onError }: { capabilities: Bootstrap["capabilities"]; onAdded: () => Promise<void>; onError: (error: unknown) => void }) {
  ...
  return <Page title="Bring in a new cut" eyebrow="Ingest desk" intro="Files are validated in staging before Spotify can see them.">
    <div className="intake-grid">
      ...
      <form className={capabilities.metube ? "intake-card" : "intake-card disabled"} onSubmit={download}><span className="card-index">URL</span><Disc3 /><h2>Through MeTube</h2><p>Request audio from your configured sidecar, then import the completed result into the same staging flow.</p>{capabilities.metube ? <><input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder="https://…" required /><button className="secondary" disabled={busy}>Request audio</button>{job && <div className="job-strip"><span>{job.status}</span>{job.status === "complete" ? <button type="button" onClick={importJob}>Import file</button> : <button type="button" onClick={checkJob}>Check progress</button>}</div>}</> : <div className="not-configured">Set the `LINER_METUBE_*` variables to enable this source.</div>}</form>
    </div>
  </Page>;
}
